...
class StaticUiContractTest(unittest.TestCase):
    ...
    def test_doi_page_summarizes_failed_links(self) -> None:
        ...
        self.assertIn('id="doiVerificationQueue"', html)
        self.assertIn('id="retryDoiVerificationQueue"', html)
        self.assertIn("未下载成功 DOI 链接", app_js)
        self.assertIn("登录/验证队列", app_js)
        self.assertIn('const doiSuccessStatuses = new Set(["downloaded", "skipped_existing"]);', app_js)
        self.assertIn("const doiQueueActionLabel = {", app_js)
        self.assertIn("function renderDoiFailedLinks(downloads)", app_js)
        self.assertIn("function renderDoiVerificationQueue(downloads)", app_js)
        self.assertIn("最近 ${esc(latestByDoi.length)} 个 DOI 的最新状态", app_js)
        self.assertIn('name="use_deepseek" checked', html)
        self.assertIn('name="campus_session_mode"', html)
        self.assertIn("校园授权会话", html)
        ...
        self.assertIn("campus_session_mode: Boolean(form.campus_session_mode.checked)", app_js)
        self.assertIn("campus_session_retry_eligible", app_js)
        self.assertIn('/api/doi-downloads/retry-verification-queue', app_js)
        self.assertIn(".doi-failed-links {", css)
        self.assertIn(".doi-log-actions {", css)
